package km.project.gsonproject.api.retofit.php.bases
...
import km.project.gsonproject.R
import km.project.gsonproject.ResponseFailedModel
import km.project.gsonproject.util.AlertUtil
import km.project.gsonproject.util.Util
import km.project.gsonproject.api.retofit.DateDeserializer
import km.project.gsonproject.api.retofit.php.APIService
...
abstract class BaseAPIController {
        ...
        val okHttpBuilder = OkHttpClient().newBuilder()
                ...
                .addInterceptor {
                    it.proceed(it.request().newBuilder().apply {
                        header(HEADER_USER_AGENT, makeUserAgent(context))
                    }.build())
                }
    ...
    private fun makeUserAgent(context: Context): String {
        val appVersion = Util.instance.getAppVersion(context)
        val osVersion = Build.VERSION.RELEASE
        return String.format(Locale.ENGLISH, "WAUG/%s (Android; %s)", appVersion, osVersion)
    }
    ...
    private fun getRetrofitWithGsonConverter(okHttpClient: OkHttpClient): Retrofit {
        return Retrofit.Builder()
                .baseUrl(BaseAPIConsts.PHP_BASE_URL)
                .client(okHttpClient)
    ...
    companion object {
        ...
        private const val HEADER_ACCEPT_CURRENCY = "Accept-Currency"
    }
}